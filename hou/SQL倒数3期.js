...
/**
 * 获取数据库中的倒数第3期开奖信息
 * @returns {Promise<Object>} 包含倒数第3期完整开奖信息的响应
 */
async function getThirdLastDatabaseDrawInfo() {
  try {
    console.log('执行数据库查询获取倒数第3期完整开奖数据');
    // 查询倒数第3期的完整开奖信息，使用正确的字段名
    // 使用LIMIT 2, 1来获取第三行数据（即倒数第3期）
    const result = await query('SELECT issue, draw_date, red, blue FROM lottery_results ORDER BY draw_date DESC LIMIT 2, 1');
    let responseData;
    ...
    console.log('获取倒数第3期完整开奖数据成功');
    console.log('返回数据:', JSON.stringify(responseData));
    
    return responseData;
  } catch (error) {
    console.error('获取倒数第3期完整开奖数据失败:', error);
...
/**
 * 获取数据库倒数第3期完整开奖信息接口
 * 接口功能: 获取数据库中存储的倒数第3期大乐透完整开奖信息，包括期号、开奖日期、前区号码和后区号码
 * 
 * 请求示例:
 * GET http://localhost:18889/sql_dao_shu_3_qi
 * 
 * 成功响应:
 * {
 *   "success": true,
...
router.get('/', async (req, res) => {
  try {
    const result = await getThirdLastDatabaseDrawInfo();
    res.json(result);
  } catch (error) {
    console.error('获取倒数第3期完整开奖数据失败:', error);
