...
/**
 * 获取倒数第2期前区三球组合下下期出现统计数据
 * @param {string} period - 统计周期（35, 50, 100, 200, 300, 500, 1000）
 * @returns {Promise<Object>} 包含最新开奖数据、倒数第2期开奖数据和三球组合统计的结果
 */
async function getDaoShu2QiTripleKillAnalysis(period) {
  try {
    // 1. 参数验证
    ...
    console.log('最新两期开奖数据查询完成');
    
    if (!latestResults || latestResults.length < 2) {
      throw new Error('未找到足够的开奖数据');
    }

    const latestDraw = latestResults[0]; // 最新一期
    const secondLastDraw = latestResults[1]; // 倒数第2期
    
    console.log('获取到最新开奖期号:', latestDraw.issue);
    console.log('获取到倒数第2期开奖期号:', secondLastDraw.issue);
    console.log('倒数第2期前区红球数据:', secondLastDraw.red);
    ...
    if (frontCombinations.length === 0) {
      throw new Error('倒数第2期前区号码不足3个，无法生成三球组合');
    }
    console.log('倒数第2期前区三球组合:', frontCombinations);
    ...
    frontCombinations.forEach(combination => {
      frontCombinationStats[combination] = {
        ...
        count: 0, // 添加组合出现次数统计
        nextNextDrawNumbers: [], // 该组合出现时下下期的号码
        numberCounts: {} // 下下期每个号码的出现次数
      };
    });
    ...
    const result = {
      ...
      secondLastDraw: {
        ...
        lastZoneNumbers: secondLastDraw.blue,
        fullData: secondLastDraw
      },
    ...
    console.error('倒数2期前区三球组合分析出错:', error);
...
/**
 * 倒数2期前区三球组合分析接口
 * @route GET /dao_shu_2_qi_san_qiu_zu_he
 * @group 数据分析 - 倒数2期前区三球组合相关接口
 * @param {string} period.query.required - 统计周期 (35, 50, 100, 200, 300, 500, 1000)
 * @returns {object} 200 - 成功响应，包含最新开奖数据、倒数第2期开奖数据和三球组合统计
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getDaoShu2QiTripleKillAnalysis(period);
