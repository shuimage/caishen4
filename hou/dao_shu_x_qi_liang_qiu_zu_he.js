...
async function getXLastFrontZoneAnalysis(period, xPeriod) {
  try {
    ...
    try {
      ...
      const processedBalls = processBalls(xLastDraw.red);
      // 转换回数字数组，并过滤掉非数字值
    ...
    console.error('倒数第X期前区两球组合分析出错:', error);
...
/**
 * 倒数第X期前区两球组合分析接口
 * @route GET /dao_shu_x_qi_liang_qiu_zu_he
 * @group 数据分析 - 倒数第X期前区两球组合分析相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getXLastFrontZoneAnalysis(period, x_period);
