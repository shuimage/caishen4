...
// 初始化后区深度预测数据表
async function initHouQuDeepPredictionTable() {
  try {
    const connection = await pool.getConnection();
    
    // 创建后区深度预测数据表
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS hou_qu_shen_du_yu_ce (
    ...
    connection.release();
    console.log('后区深度预测数据表初始化成功');
  } catch (error) {
    console.error('初始化后区深度预测数据表失败:', error);
  }
}

// 调用初始化函数
initHouQuDeepPredictionTable();
  ...
  try {
    ...
    try {
      ...
      for (const result of backtest_results) {
        ...
        await connection.execute(
          `REPLACE INTO hou_qu_shen_du_yu_ce (period, backtest_period, card_index, card_name, buy_numbers, accuracy_data)
           VALUES (?, ?, ?, ?, ?, ?)`,
    ...
    console.error('保存后区深度预测数据失败:', error);
...
router.get('/get_shen_du_yu_ce_data', async (req, res) => {
  try {
    ...
    let query = 'SELECT * FROM hou_qu_shen_du_yu_ce WHERE 1=1';
    const params = [];
    ...
    console.error('获取后区深度预测数据失败:', error);
    res.json({ success: false, message: '接口失败' });
  }
});
...
router.delete('/delete_shen_du_yu_ce_data', async (req, res) => {
  try {
    ...
    let query = 'DELETE FROM hou_qu_shen_du_yu_ce WHERE period = ?';
    const params = [period];
    ...
    console.error('删除后区深度预测数据失败:', error);
    res.json({ success: false, message: '接口失败' });
  }
});
