...
// 接口功能: 回测倒数2期三球组合前区买号结果，比较推荐买号与真实的开奖结果
 ...
 */
async function huo_qu_dao_shu_2_qi_san_qiu_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most') {
  try {
    // 验证参数
    ...
    if (backtest_period === 'all') {
      ...
      // 查询指定周期内的历史数据，多获取2期用于倒数2期分析
      const limit = parseInt(backtest_period);
      const historySql = `
        SELECT * 
        FROM lottery_results 
        ORDER BY issue DESC 
        LIMIT ${limit + 2}  -- 多获取2期，用于倒数2期回测比较
      `;
      historyData = await query(historySql);
    }
    ...
    for (let i = 2; i < historyData.length; i++) {
      ...
      backtestResults.push({
        ...
        nextFrontNumbers: nextFrontNumbers, // 下期实际开奖前区
...
/**
 * 倒数2期三球组合前区买号回测接口
 * @route GET /dao_shu_2_qi_san_qiu_mai_hao_hui_ce
 * @group 数据分析 - 买号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await huo_qu_dao_shu_2_qi_san_qiu_mai_hao_hui_ce(backtest_period, stats_period, backtest_method);
