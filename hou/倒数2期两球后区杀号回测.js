 ...
 */
async function getDaoShu2QiHouQuKillNumberBacktest(backtest_period, stats_period) {
  try {
    // 验证参数
    ...
    for (let i = 2; i < historyData.length; i++) {
      ...
      for (let j = 2; j < statsHistoryData.length; j++) {
        ...
        const statsNextNextDraw = statsHistoryData[j - 2]; // 下下期
          ...
          if (backCombinations.includes(comb)) {
            ...
            try {
              if (Array.isArray(statsNextNextDraw.blue)) {
                nextNextNumbers = statsNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextDraw.blue === 'string') {
                nextNextNumbers = statsNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
      ...
      recommendedBackKillNumbers.forEach(killNum => {
        if (!nextNextBackNumbers.includes(killNum)) {
      ...
      backtestResults.push({
        ...
        nextNextBackNumbers: nextNextBackNumbers, // 下下期实际开奖后区
...
/**
 * 倒数2期两球组合后区杀号回测接口
 * @route GET /dao_shu_2_qi_hou_qu_sha_hao_hui_ce
 * @group 数据分析 - 杀号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getDaoShu2QiHouQuKillNumberBacktest(backtest_period, stats_period);
