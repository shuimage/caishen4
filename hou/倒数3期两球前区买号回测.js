...
// 接口功能: 回测倒数3期两球组合前区买号结果，比较推荐买号与真实的开奖结果
 ...
 */
async function dao_shu_3_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most', target_period = null) {
  try {
    // 验证参数
    ...
    if (target_period) {
      ...
      const historySql = `
        ...
        ORDER BY issue DESC 
        LIMIT ${backtest_period === 'all' ? 1000 : parseInt(backtest_period) + 3}  -- 多获取3期，用于回测比较（需要下下下期数据）
      `;
      ...
      const historySql = `
        ...
        FROM lottery_results 
        ORDER BY issue DESC 
        LIMIT ${limit + 3}  -- 多获取3期，用于回测比较（需要下下下期数据）
      `;
      historyData = await query(historySql);
    }
    ...
    for (let i = 3; i < historyData.length; i++) {
      ...
      recommendedFrontBuyNumbers.forEach(buyNum => {
        if (nextNextNextFrontNumbers.includes(buyNum)) {
...
/**
 * 倒数3期两球组合前区买号回测接口
 * @route GET /dao_shu_3_qi_mai_hao_hui_ce
 * @group 数据分析 - 买号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await dao_shu_3_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method, target_period);
