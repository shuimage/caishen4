 ...
 */
async function getDaoShu3QiKillNumberBacktest(backtest_period, stats_period) {
  try {
    // 验证参数
    ...
    for (let i = 3; i < historyData.length; i++) {
      ...
      // 解析下下下期的前区号码
      let nextNextNextFrontNumbers = [];
      try {
        if (Array.isArray(nextNextNextDraw.red)) {
          nextNextNextFrontNumbers = nextNextNextDraw.red.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof nextNextNextDraw.red === 'string') {
          const numbers = nextNextNextDraw.red.match(/\d+/g);
          if (numbers) {
            nextNextNextFrontNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
      ...
      recommendedFrontKillNumbers.forEach(killNum => {
        if (!nextNextNextFrontNumbers.includes(killNum)) {
      ...
      backtestResults.push({
        ...
        nextNextNextFrontNumbers: nextNextNextFrontNumbers, // 下下下期实际开奖前区
...
/**
 * 倒数3期两球组合前区杀号回测接口
 * @route GET /dao_shu_3_qi_sha_hao_hui_ce
 * @group 数据分析 - 杀号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getDaoShu3QiKillNumberBacktest(backtest_period, stats_period);
