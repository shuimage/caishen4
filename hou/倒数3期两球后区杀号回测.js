 ...
 */
async function getDaoShu3QiHouQuKillNumberBacktest(backtest_period, stats_period) {
  try {
    // 验证参数
    ...
    for (let i = 3; i < historyData.length; i++) {
      ...
      const nextNextNextDraw = historyData[i - 3]; // 下下下期（用于比较的实际开奖期数）
      ...
      for (let j = 3; j < statsHistoryData.length; j++) {
        ...
        const statsNextNextNextDraw = statsHistoryData[j - 3]; // 下下下期
          ...
          if (backCombinations.includes(comb)) {
            ...
            try {
              if (Array.isArray(statsNextNextNextDraw.blue)) {
                nextNextNumbers = statsNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextDraw.blue === 'string') {
                nextNextNumbers = statsNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
      ...
      recommendedBackKillNumbers.forEach(killNum => {
        if (!nextNextNextBackNumbers.includes(killNum)) {
          // 正确杀号：推荐杀号没有出现在下下下期开奖号中
          backCorrectKill++;
        } else {
          // 错误杀号：推荐杀号出现在下下下期开奖号中
          backWrongKill++;
        }
      });
      ...
      backtestResults.push({
        ...
        nextNextNextBackNumbers: nextNextNextBackNumbers, // 下下下期实际开奖后区
...
/**
 * 倒数3期两球组合后区杀号回测接口
 * @route GET /dao_shu_3_qi_hou_qu_sha_hao_hui_ce
 * @group 数据分析 - 杀号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getDaoShu3QiHouQuKillNumberBacktest(backtest_period, stats_period);
