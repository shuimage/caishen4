...
async function getThirdLastBackZoneAnalysis(period) {
  try {
    ...
    const thirdLastResult = await query(thirdLastResultSql);
    ...
    console.log('蓝球数据:', thirdLastDraw.blue);
    
    // 解析后区号码（从blue字段中获取）
    let backNumbers = [];
    try {
      if (Array.isArray(thirdLastDraw.blue)) {
        // 如果已经是数组，直接使用
        backNumbers = thirdLastDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
      } else if (typeof thirdLastDraw.blue === 'string') {
        // 从字符串中提取数字
        const numbers = thirdLastDraw.blue.match(/\d+/g);
        if (numbers) {
      ...
      if (!backNumbers.length) {
        console.error('Failed to extract back numbers from:', thirdLastDraw.blue);
      }
    } catch (e) {
        ...
        if (backCombinations.includes(comb)) {
          ...
          try {
            if (Array.isArray(nextNextNextDraw.blue)) {
              nextNextNextNumbers = nextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
            } else if (typeof nextNextNextDraw.blue === 'string') {
              nextNextNextNumbers = nextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
            }
          } catch (e) {
            console.error('Error extracting next next next draw blue numbers:', e);
          }
          ...
          backCombinationStats[comb].occurrenceDetails.push({
            ...
            nextNextNextBackNumbers: nextNextNextNumbers
          });
    ...
    console.error('倒数第3期后区两球组合分析出错:', error);
...
/**
 * 倒数第3期后区两球组合分析接口
 * @route GET /dao_shu_3_qi_hou_qu_zu_he
 * @group 数据分析 - 倒数第3期后区两球组合分析相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getThirdLastBackZoneAnalysis(period);
