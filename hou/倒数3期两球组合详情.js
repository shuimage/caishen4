// 倒数3期两球组合详情服务器 - 仅作为模块导出，不再单独启动服务器
...
app.post('/dao_shu_3_qi_liang_qiu_zu_he_xiang_qing', async (req, res) => {
  try {
    ...
    // 查询倒数3期的开奖数据
    const daoShu3QiData = await query(
      'SELECT * FROM lottery_results WHERE issue = ? LIMIT 1',
      [daoShu3QiPeriod]
    );
    
    if (!daoShu3QiData || daoShu3QiData.length === 0) {
      return res.status(404).json({ code: 404, message: '未找到倒数3期的开奖数据' });
    }
    ...
    if (matchingCombinations.length === 0) {
      return res.status(404).json({ code: 404, message: '在倒数3期中未找到匹配的组合' });
    }
    
    // 查询历史数据，查找这些组合出现的记录，获取下下下期的数据
    // 构建查询SQL
    let sql = `
      SELECT 
        l1.id, 
        ...
        l2.issue as next_next_next_period,
        l2.${drawField} as next_next_next_draw_info
      FROM 
        lottery_results l1
      LEFT JOIN 
        lottery_results l2 ON l1.id + 3 = l2.id
      WHERE 
        l1.id < ?
    `;
    ...
    // 构建查询参数
    const params = [daoShu3QiResult.id];
