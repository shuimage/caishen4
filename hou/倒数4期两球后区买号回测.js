...
// 接口功能: 回测倒数4期两球组合后区买号结果，比较推荐买号与真实的下下下下期开奖结果
// 返回每一期的回测数据，包括期号、推荐买号、下下下下期开奖号、正确买号和错误买号数量
 ...
 */
async function huo_qu_dao_shu_4_qi_hou_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most') {
  try {
    // 验证参数
    ...
    // 处理旧的回测方法参数，向后兼容
    if (backtest_method === 'most' || backtest_method === 'least' || backtest_method === 'average') {
      backtest_method = 'rank1';
      console.log('将旧的回测方法参数转换为:', backtest_method);
    }
    
    // 验证回测方法参数
    const isValidRankMethod = /^rank(1[0-2]|[1-9])$/.test(backtest_method);
    if (!isValidRankMethod) {
      throw new Error('无效的回测方法参数，必须是 rank1-rank12 中的一个');
    }
    ...
    // 对每一期进行回测（跳过最新4期，因为没有下下下下期数据）
    for (let i = 4; i < historyData.length; i++) {
      const currentDraw = historyData[i];  // 当前回测期
      const nextNextNextNextDraw = historyData[i - 4]; // 下下下下期开奖结果（用于比较）
      ...
      // 遍历统计历史数据，查找组合出现的位置，并记录下下下下期的号码
      for (let j = 4; j < statsHistoryData.length; j++) {
        const statsCurrentDraw = statsHistoryData[j];
        const statsNextNextNextNextDraw = statsHistoryData[j - 4];
        ...
        statsCurrentBackCombinations.forEach(comb => {
          if (backCombinations.includes(comb)) {
            ...
            let nextNumbers = [];
            try {
              if (Array.isArray(statsNextNextNextNextDraw.blue)) {
                nextNumbers = statsNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextNextDraw.blue === 'string') {
                nextNumbers = statsNextNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
              console.error('解析统计下下下下期后区号码失败:', e);
              return;
            }
      ...
      // 解析下下下下期的后区号码
      let nextNextNextNextBackNumbers = [];
      try {
        if (Array.isArray(nextNextNextNextDraw.blue)) {
          nextNextNextNextBackNumbers = nextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof nextNextNextNextDraw.blue === 'string') {
          const numbers = nextNextNextNextDraw.blue.match(/\d+/g);
          if (numbers) {
            nextNextNextNextBackNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
        console.error('解析下下下下期后区号码失败:', e);
        continue;
      }
      ...
      recommendedBackBuyNumbers.forEach(buyNum => {
        if (nextNextNextNextBackNumbers.includes(buyNum)) {
          // 正确买号：推荐买号出现在下下下下期开奖号中
          backCorrectBuy++;
        } else {
          // 错误买号：推荐买号没有出现在下下下下期开奖号中
          backWrongBuy++;
        }
      });
      ...
      backtestResults.push({
        ...
        nextNextNextNextIssue: nextNextNextNextDraw.issue, // 下下下下期开奖期号
        nextNextNextNextBackNumbers: nextNextNextNextBackNumbers, // 下下下下期实际开奖后区
...
/**
 * 倒数4期两球组合后区买号回测接口
 * @route GET /huo_qu_dao_shu_4_qi_hou_mai_hao_hui_ce
 * @group 数据分析 - 买号回测相关接口
 ...
 * @param {string} backtest_method.query - 回测方法 (rank1-rank12)，默认 rank1
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await huo_qu_dao_shu_4_qi_hou_mai_hao_hui_ce(backtest_period, stats_period, backtest_method);
