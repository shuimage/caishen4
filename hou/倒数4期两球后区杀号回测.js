...
// 返回每一期的回测数据，包括期号、推荐杀号、下下下期数据，正确杀号和错误杀号数量
 ...
 */
async function getDaoShu4QiHouQuKillNumberBacktest(backtest_period, stats_period) {
  try {
    // 验证参数
    ...
    for (let i = 4; i < historyData.length; i++) {
      ...
      const nextNextNextNextDraw = historyData[i - 4]; // 下下下下期（用于比较的实际开奖期数）
      ...
      for (let j = 4; j < statsHistoryData.length; j++) {
        ...
        const statsNextNextNextNextDraw = statsHistoryData[j - 4]; // 下下下下期
          ...
          if (backCombinations.includes(comb)) {
            ...
            try {
              if (Array.isArray(statsNextNextNextNextDraw.blue)) {
                nextNextNextNumbers = statsNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextNextDraw.blue === 'string') {
                nextNextNextNumbers = statsNextNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
      ...
      recommendedBackKillNumbers.forEach(killNum => {
        if (!nextNextNextNextBackNumbers.includes(killNum)) {
          // 正确杀号：推荐杀号没有出现在下下下下期开奖号中
          backCorrectKill++;
        } else {
          // 错误杀号：推荐杀号出现在下下下下期开奖号中
          backWrongKill++;
        }
      });
...
/**
 * 倒数4期两球组合后区杀号回测接口
 * @route GET /dao_shu_4_qi_hou_qu_sha_hao_hui_ce
 * @group 数据分析 - 杀号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getDaoShu4QiHouQuKillNumberBacktest(backtest_period, stats_period);
