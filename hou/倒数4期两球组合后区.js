...
async function getFourthLastBackZoneAnalysis(period) {
  try {
    ...
    console.log('蓝球数据:', fourthLastDraw.blue);
    ...
    try {
      if (Array.isArray(fourthLastDraw.blue)) {
        // 如果已经是数组，直接使用
        backNumbers = fourthLastDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
      ...
      if (!backNumbers.length) {
        console.error('Failed to extract back numbers from:', fourthLastDraw.blue);
      }
    } catch (e) {
        ...
        if (backCombinations.includes(comb)) {
          ...
          try {
            if (Array.isArray(nextNextNextNextDraw.blue)) {
              nextNextNextNextNumbers = nextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
            } else if (typeof nextNextNextNextDraw.blue === 'string') {
              nextNextNextNextNumbers = nextNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
            }
          } catch (e) {
    ...
    const result = {
      fourthLastDraw: {
        ...
        drawDate: fourthLastDraw.draw_date || fourthLastDraw.date || new Date().toISOString().split('T')[0],
        firstZoneNumbers: [],
    ...
    console.error('倒数第4期后区两球组合分析出错:', error);
...
/**
 * 倒数第4期后区两球组合分析接口
 * @route GET /dao_shu_4_qi_hou_qu_zu_he
 * @group 数据分析 - 倒数第4期后区两球组合分析相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getFourthLastBackZoneAnalysis(period);
