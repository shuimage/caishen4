...
// 接口功能: 回测倒数5期两球组合前区买号结果，比较推荐买号与真实的开奖结果
// 返回每一期的回测数据，包括期号、推荐买号、下下下下下期开奖号、正确买号和错误买号数量
 ...
 */
async function dao_shu_5_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most', target_period = null) {
  try {
    // 验证参数
    ...
    if (target_period) {
      ...
      const historySql = `
        ...
        ORDER BY issue DESC 
        LIMIT ${backtest_period === 'all' ? 1000 : parseInt(backtest_period) + 5}  -- 多获取5期，用于回测比较（需要下下下下下期数据）
      `;
      ...
      const historySql = `
        ...
        FROM lottery_results 
        ORDER BY issue DESC 
        LIMIT ${limit + 5}  -- 多获取5期，用于回测比较（需要下下下下下期数据）
      `;
      historyData = await query(historySql);
    }
    ...
    // 对每一期进行回测（跳过最新5期，因为没有下下下下下期数据）
    for (let i = 5; i < historyData.length; i++) {
      const currentDraw = historyData[i];  // 当前回测期
      const nextNextNextNextNextDraw = historyData[i - 5]; // 下下下下下期开奖结果（用于比较）
      ...
      for (let j = 5; j < statsHistoryData.length; j++) {
        ...
        const statsNextNextNextNextNextDraw = statsHistoryData[j - 5];
        ...
        statsCurrentFrontCombinations.forEach(comb => {
          if (frontCombinations.includes(comb)) {
            ...
            try {
              if (Array.isArray(statsNextNextNextNextNextDraw.red)) {
                nextNextNextNextNextNumbers = statsNextNextNextNextNextDraw.red.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextNextNextDraw.red === 'string') {
                nextNextNextNextNextNumbers = statsNextNextNextNextNextDraw.red.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
              console.error('解析统计下下下下下期前区号码失败:', e);
              return;
            }
      ...
      // 解析下下下下下期的前区号码
      let nextNextNextNextNextFrontNumbers = [];
      try {
        if (Array.isArray(nextNextNextNextNextDraw.red)) {
          nextNextNextNextNextFrontNumbers = nextNextNextNextNextDraw.red.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof nextNextNextNextNextDraw.red === 'string') {
          const numbers = nextNextNextNextNextDraw.red.match(/\d+/g);
          if (numbers) {
            nextNextNextNextNextFrontNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
        console.error('解析下下下下下期前区号码失败:', e);
        continue;
      }
      ...
      recommendedFrontBuyNumbers.forEach(buyNum => {
        if (nextNextNextNextNextFrontNumbers.includes(buyNum)) {
      ...
      backtestResults.push({
        ...
        nextNextNextNextNextFrontNumbers: nextNextNextNextNextFrontNumbers, // 下下下下下期实际开奖前区
...
/**
 * 倒数5期两球组合前区买号回测接口
 * @route GET /dao_shu_5_qi_mai_hao_hui_ce
 * @group 数据分析 - 买号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await dao_shu_5_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method, target_period);
