...
// 接口功能: 回测统计范围内的推荐杀号，比较推荐杀号与真实的开奖结果
// 返回每一期的回测数据，包括期号、推荐杀号、下下下下下期开奖号、正确杀号和错误杀号数量
 ...
 */
async function getDaoShu5QiKillNumberBacktest(backtest_period, stats_period) {
  try {
    // 验证参数
    ...
    for (let i = 5; i < historyData.length; i++) {
      ...
      recommendedFrontKillNumbers.forEach(killNum => {
        if (!nextNextNextNextNextFrontNumbers.includes(killNum)) {
...
/**
 * 倒数5期两球组合前区杀号回测接口
 * @route GET /dao_shu_5_qi_sha_hao_hui_ce
 * @param {string} backtest_period - 回测周期
 * @param {string} stats_period - 统计周期
 * @returns {Object} 回测结果
 */
router.get('/', async (req, res) => {
  try {
    const { backtest_period = '100', stats_period = '100' } = req.query;
    
    console.log('收到倒数5期杀号回测请求:', {
      backtest_period,
      stats_period
    });
    
    const result = await getDaoShu5QiKillNumberBacktest(backtest_period, stats_period);
    
    res.status(200).json(result);
  } catch (error) {
    console.error('倒数5期杀号回测出错:', error);
    res.status(500).json({
      success: false,
