...
// 接口功能: 回测倒数5期两球组合后区买号结果，比较推荐买号与真实的下下下下下期开奖结果
 ...
 */
async function huo_qu_dao_shu_5_qi_hou_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most') {
  try {
    // 验证参数
          ...
          if (backCombinations.includes(comb)) {
            ...
            try {
              if (Array.isArray(statsNextNextNextNextNextDraw.blue)) {
                nextNumbers = statsNextNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextNextNextDraw.blue === 'string') {
                nextNumbers = statsNextNextNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
...
/**
 * 倒数5期两球组合后区买号回测接口
 * @route GET /huo_qu_dao_shu_5_qi_hou_mai_hao_hui_ce
 * @group 数据分析 - 买号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await huo_qu_dao_shu_5_qi_hou_mai_hao_hui_ce(backtest_period, stats_period, backtest_method);
