...
// 接口功能: 回测统计范围内的推荐杀号，比较推荐杀号与真实开奖结果
// 返回每一期的回测数据，包括期号、推荐杀号、下下下下下期数据，正确杀号和错误杀号数量
 ...
 */
async function getDaoShu5QiHouQuKillNumberBacktest(backtest_period, stats_period) {
  try {
    // 验证参数
    ...
    for (let i = 5; i < historyData.length; i++) {
      ...
      const nextNextNextDraw = historyData[i - 3]; // 下下下期
      const nextNextNextNextDraw = historyData[i - 4]; // 下下下下期
      const nextNextNextNextNextDraw = historyData[i - 5]; // 下下下下下期（用于比较的实际开奖期数）
      ...
      // 遍历统计历史数据，查找组合出现的位置，并记录下下下下下期的号码
      for (let j = 5; j < statsHistoryData.length; j++) {
        const statsCurrentDraw = statsHistoryData[j];
        const statsNextNextNextNextNextDraw = statsHistoryData[j - 5]; // 下下下下下期
        ...
        statsCurrentBackCombinations.forEach(comb => {
          if (backCombinations.includes(comb)) {
            ...
            try {
              if (Array.isArray(statsNextNextNextNextNextDraw.blue)) {
                nextNextNextNextNextNumbers = statsNextNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextNextNextDraw.blue === 'string') {
                nextNextNextNextNextNumbers = statsNextNextNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
              console.error('解析统计下下下下下期后区号码失败:', e);
              return;
            }
      ...
      try {
        if (Array.isArray(nextNextNextNextNextDraw.blue)) {
          nextNextNextNextNextBackNumbers = nextNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof nextNextNextNextNextDraw.blue === 'string') {
          const numbers = nextNextNextNextNextDraw.blue.match(/\d+/g);
          if (numbers) {
            nextNextNextNextNextBackNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
        console.error('解析下下下下下期后区号码失败:', e);
        continue;
      }
      ...
      recommendedBackKillNumbers.forEach(killNum => {
        if (!nextNextNextNextNextBackNumbers.includes(killNum)) {
          // 正确杀号：推荐杀号没有出现在下下下下下期开奖号中
          backCorrectKill++;
        } else {
          // 错误杀号：推荐杀号出现在下下下下下期开奖号中
          backWrongKill++;
        }
      });
      ...
      backtestResults.push({
        ...
        nextNextNextNextNextIssue: nextNextNextNextNextDraw.issue, // 下下下下下期开奖期号
        nextNextNextNextNextBackNumbers: nextNextNextNextNextBackNumbers, // 下下下下下期实际开奖后区
        backCorrectKill: backCorrectKill, // 后区正确杀号数量
        backWrongKill: backWrongKill // 后区错误杀号数量
      });
    }
...
/**
 * 倒数5期两球组合后区杀号回测接口
 * @route GET /dao_shu_5_qi_hou_qu_sha_hao_hui_ce
 * @group 数据分析 - 杀号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getDaoShu5QiHouQuKillNumberBacktest(backtest_period, stats_period);
