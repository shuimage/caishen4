...
async function getFifthLastBackZoneAnalysis(period) {
  try {
    ...
    console.log('蓝球数据:', fifthLastDraw.blue);
    
    // 解析后区号码（从blue字段中获取）
    let backNumbers = [];
    try {
      if (Array.isArray(fifthLastDraw.blue)) {
        // 如果已经是数组，直接使用
        backNumbers = fifthLastDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
      } else if (typeof fifthLastDraw.blue === 'string') {
        // 从字符串中提取数字
        const numbers = fifthLastDraw.blue.match(/\d+/g);
        if (numbers) {
      ...
      if (!backNumbers.length) {
        console.error('Failed to extract back numbers from:', fifthLastDraw.blue);
      }
    } catch (e) {
    ...
    if (period === 'all') {
      ...
      const historySql = `
        SELECT * 
        FROM lottery_results 
        WHERE issue < '${fifthLastDraw.issue}' 
        ORDER BY issue DESC
      `;
    ...
    // 注意：历史数据是按issue降序排列的，所以下下下下下期的索引是i-5
    for (let i = 5; i < historyData.length; i++) {
      const currentDraw = historyData[i];  // 当前期（组合出现的期数）
      const nextNextNextNextNextDraw = historyData[i - 5]; // 下下下下下期（期号比当前期大5，因为数据是降序排列的）
      
      // 确保nextNextNextNextNextDraw存在
        ...
        if (backCombinations.includes(comb)) {
          ...
          try {
            if (Array.isArray(nextNextNextNextNextDraw.blue)) {
              nextNextNextNextNextNumbers = nextNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
            } else if (typeof nextNextNextNextNextDraw.blue === 'string') {
              nextNextNextNextNextNumbers = nextNextNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
            }
          } catch (e) {
            console.error('Error extracting next next next next next draw blue numbers:', e);
          }
          ...
          backCombinationStats[comb].occurrenceDetails.push({
            ...
            nextNextNextNextNextBackNumbers: nextNextNextNextNextNumbers
          });
    ...
    console.error('倒数第5期后区两球组合分析出错:', error);
...
/**
 * 倒数第5期后区两球组合分析接口
 * @route GET /dao_shu_5_qi_hou_qu_zu_he
 * @group 数据分析 - 倒数第5期后区两球组合分析相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getFifthLastBackZoneAnalysis(period);
