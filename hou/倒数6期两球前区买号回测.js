...
// 接口功能: 回测倒数6期两球组合前区买号结果，比较推荐买号与真实的开奖结果
 ...
 */
async function dao_shu_6_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most') {
  try {
    // 解析参数
    ...
    if (parsedBacktestPeriod === 'all') {
      ...
      const historySql = `
        ...
        FROM lottery_results 
        ORDER BY issue DESC 
        LIMIT ${limit + 6}  -- 多获取6期，用于回测比较（需要下下下下下下下期数据）
      `;
      historyData = await query(historySql);
    }
    ...
    // 对每一期进行回测（跳过最新6期，因为没有下下下下下下下期数据）
    for (let i = 6; i < historyData.length; i++) {
      const currentDraw = historyData[i];  // 当前回测期
      const nextNextNextNextNextNextDraw = historyData[i - 6]; // 下下下下下下下期开奖结果（用于比较）
      ...
      for (let j = 6; j < statsHistoryData.length; j++) {
        ...
        statsCurrentFrontCombinations.forEach(comb => {
          if (frontCombinations.includes(comb)) {
            ...
            try {
              if (Array.isArray(statsNextNextNextNextNextNextDraw.red)) {
                nextNextNextNextNextNextNumbers = statsNextNextNextNextNextNextDraw.red.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextNextNextNextDraw.red === 'string') {
                nextNextNextNextNextNextNumbers = statsNextNextNextNextNextNextDraw.red.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
              console.error('解析统计下下下下下下下期前区号码失败:', e);
              return;
            }
      ...
      // 解析下下下下下下下期的前区号码
      let nextNextNextNextNextNextFrontNumbers = [];
      try {
        if (Array.isArray(nextNextNextNextNextNextDraw.red)) {
          nextNextNextNextNextNextFrontNumbers = nextNextNextNextNextNextDraw.red.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof nextNextNextNextNextNextDraw.red === 'string') {
          const numbers = nextNextNextNextNextNextDraw.red.match(/\d+/g);
          if (numbers) {
            nextNextNextNextNextNextFrontNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
        console.error('解析下下下下下下下期前区号码失败:', e);
        continue;
      }
      ...
      recommendedFrontBuyNumbers.forEach(buyNum => {
        if (nextNextNextNextNextNextFrontNumbers.includes(buyNum)) {
      ...
      backtestResults.push({
        ...
        nextNextNextNextNextNextIssue: nextNextNextNextNextNextDraw.issue, // 下下下下下下下期开奖期号
        nextNextNextNextNextNextFrontNumbers: nextNextNextNextNextNextFrontNumbers, // 下下下下下下下期实际开奖前区
...
/**
 * 倒数6期两球组合前区买号回测接口
 * @route GET /dao_shu_6_qi_mai_hao_hui_ce
 * @group 数据分析 - 买号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await dao_shu_6_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method);
