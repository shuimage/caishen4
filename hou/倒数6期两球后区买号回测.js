...
// 接口功能: 回测倒数6期两球组合后区买号结果，比较推荐买号与真实的开奖结果
// 返回每一期的回测数据，包括期号、推荐买号、下下下下下下期开奖号、正确买号和错误买号数量
 ...
 */
async function dao_shu_6_qi_hou_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most') {
  try {
    // 解析参数
    ...
    if (parsedBacktestPeriod === 'all') {
      ...
      const historySql = `
        ...
        FROM lottery_results 
        ORDER BY issue DESC 
        LIMIT ${limit + 6}  -- 多获取6期，用于回测比较（需要下下下下下下期数据）
      `;
      historyData = await query(historySql);
    }
    ...
    // 对每一期进行回测（跳过最新6期，因为没有下下下下下下期数据）
    for (let i = 6; i < historyData.length; i++) {
      const currentDraw = historyData[i];  // 当前回测期
      const nextNextNextNextNextNextDraw = historyData[i - 6]; // 下下下下下下期开奖结果（用于比较）
      ...
      // 遍历统计历史数据，查找组合出现的位置，并记录下下下下下下期的号码
      for (let j = 6; j < statsHistoryData.length; j++) {
        const statsCurrentDraw = statsHistoryData[j];
        const statsNextNextNextNextNextNextDraw = statsHistoryData[j - 6];
        ...
        statsCurrentBackCombinations.forEach(comb => {
          if (backCombinations.includes(comb)) {
            ...
            try {
              if (Array.isArray(statsNextNextNextNextNextNextDraw.blue)) {
                nextNextNextNextNextNextNumbers = statsNextNextNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextNextNextNextDraw.blue === 'string') {
                nextNextNextNextNextNextNumbers = statsNextNextNextNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
              console.error('解析统计下下下下下下期后区号码失败:', e);
              return;
            }
      ...
      // 解析下下下下下下期的后区号码
      let nextNextNextNextNextNextBackNumbers = [];
      try {
        if (Array.isArray(nextNextNextNextNextNextDraw.blue)) {
          nextNextNextNextNextNextBackNumbers = nextNextNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof nextNextNextNextNextNextDraw.blue === 'string') {
          const numbers = nextNextNextNextNextNextDraw.blue.match(/\d+/g);
          if (numbers) {
            nextNextNextNextNextNextBackNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
        console.error('解析下下下下下下期后区号码失败:', e);
        continue;
      }
      ...
      recommendedBackBuyNumbers.forEach(buyNum => {
        if (nextNextNextNextNextNextBackNumbers.includes(buyNum)) {
          // 正确买号：推荐买号出现在下下下下下下期开奖号中
          backCorrectBuy++;
        } else {
          // 错误买号：推荐买号没有出现在下下下下下下期开奖号中
          backWrongBuy++;
        }
      });
      ...
      backtestResults.push({
        ...
        nextNextNextNextNextNextIssue: nextNextNextNextNextNextDraw.issue, // 下下下下下下期开奖期号
        nextNextNextNextNextNextBackNumbers: nextNextNextNextNextNextBackNumbers, // 下下下下下下期实际开奖后区
...
/**
 * 倒数6期两球组合后区买号回测接口
 * @route GET /dao_shu_6_qi_mai_hao_hui_ce_hou
 * @group 数据分析 - 买号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await dao_shu_6_qi_hou_mai_hao_hui_ce(backtest_period, stats_period, backtest_method);
