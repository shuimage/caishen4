...
async function getSixthLastBackZoneAnalysis(period) {
  try {
    ...
    // 获取倒数第6期的开奖数据
    console.log('开始查询倒数第6期开奖数据...');
    const sixthLastResultSql = `
      SELECT * 
    ...
    const sixthLastResult = await query(sixthLastResultSql);
    console.log('倒数第6期开奖数据查询完成');
    
    if (!sixthLastResult || sixthLastResult.length === 0) {
      throw new Error('未找到倒数第6期开奖数据');
    }

    const sixthLastDraw = sixthLastResult[0];
    console.log('获取到倒数第6期开奖期号:', sixthLastDraw.issue);
    console.log('蓝球数据:', sixthLastDraw.blue);
    
    // 解析后区号码（从blue字段中获取）
    let backNumbers = [];
    try {
      if (Array.isArray(sixthLastDraw.blue)) {
        // 如果已经是数组，直接使用
        backNumbers = sixthLastDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
      } else if (typeof sixthLastDraw.blue === 'string') {
        // 从字符串中提取数字
        const numbers = sixthLastDraw.blue.match(/\d+/g);
        if (numbers) {
      ...
      if (!backNumbers.length) {
        console.error('Failed to extract back numbers from:', sixthLastDraw.blue);
      }
    } catch (e) {
    ...
    if (processedPeriod === 'all') {
      ...
      const historySql = `
        SELECT * 
        FROM lottery_results 
        WHERE issue < '${sixthLastDraw.issue}' 
        ORDER BY issue DESC
      `;
    ...
    // 统计后区每个组合出现时下下下下下下期的号码
    const backCombinationStats = {};
    backCombinations.forEach(combination => {
      backCombinationStats[combination] = {
        combo: combination,
        count: 0,
        nextDrawNumbers: [], // 存储这个组合出现时下下下下下下期开出的号码
        numberCounts: {}, // 统计每个号码出现的次数
        occurrenceDetails: [] // 存储组合出现时的期号和下下下下下下期信息
      };
    });

    // 遍历历史数据，查找组合出现的位置，并记录下下下下下下期的号码
    // 注意：历史数据是按issue降序排列的，所以下下下下下下期的索引是i-6
    for (let i = 6; i < historyData.length; i++) {
      const currentDraw = historyData[i];  // 当前期（组合出现的期数）
      const nextNextNextNextNextNextDraw = historyData[i - 6]; // 下下下下下下期（期号比当前期大6，因为数据是降序排列的）
      
      // 确保nextNextNextNextNextNextDraw存在
      if (!nextNextNextNextNextNextDraw) continue;
      ...
      currentBackCombinations.forEach(comb => {
        if (backCombinations.includes(comb)) {
          // 获取下下下下下下期的后区号码
          let nextNextNextNextNextNextNumbers = [];
          try {
            if (Array.isArray(nextNextNextNextNextNextDraw.blue)) {
              nextNextNextNextNextNextNumbers = nextNextNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
            } else if (typeof nextNextNextNextNextNextDraw.blue === 'string') {
              nextNextNextNextNextNextNumbers = nextNextNextNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
            }
          } catch (e) {
            console.error('Error extracting next next next next next next draw blue numbers:', e);
          }
          
          // 记录下下下下下下期的号码
          nextNextNextNextNextNextNumbers.forEach(num => {
          ...
          backCombinationStats[comb].occurrenceDetails.push({
            ...
            nextNextNextNextNextNextPeriod: nextNextNextNextNextNextDraw.issue,
            nextNextNextNextNextNextDrawDate: nextNextNextNextNextNextDraw.draw_date || nextNextNextNextNextNextDraw.date || '',
            nextNextNextNextNextNextBackNumbers: nextNextNextNextNextNextNumbers
          });
    ...
    const result = {
      sixthLastDraw: {
        issue: sixthLastDraw.issue,
        drawDate: sixthLastDraw.draw_date || sixthLastDraw.date || new Date().toISOString().split('T')[0],
    ...
    console.error('倒数第6期后区两球组合分析出错:', error);
...
/**
 * 倒数第6期后区两球组合分析接口
 * @route GET /dao_shu_6_qi_hou_qu_zu_he
 * @group 数据分析 - 倒数第6期后区两球组合分析相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getSixthLastBackZoneAnalysis(processedPeriod);
