...
// 接口功能: 回测倒数7期两球组合前区买号结果，比较推荐买号与真实的开奖结果
 ...
 */
async function dao_shu_7_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most') {
  try {
    // 解析参数
    ...
    for (let i = 7; i < historyData.length; i++) {
      ...
      for (let j = 7; j < statsHistoryData.length; j++) {
        ...
        statsCurrentFrontCombinations.forEach(comb => {
          if (frontCombinations.includes(comb)) {
            ...
            try {
              if (Array.isArray(statsNextNextNextNextNextNextNextDraw.red)) {
                nextNextNextNextNextNextNextNumbers = statsNextNextNextNextNextNextNextDraw.red.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextNextNextNextNextDraw.red === 'string') {
                nextNextNextNextNextNextNextNumbers = statsNextNextNextNextNextNextNextDraw.red.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
      ...
      // 解析下下下下下下下期的前区号码
      let nextNextNextNextNextNextNextFrontNumbers = [];
      try {
        if (Array.isArray(nextNextNextNextNextNextNextDraw.red)) {
          nextNextNextNextNextNextNextFrontNumbers = nextNextNextNextNextNextNextDraw.red.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof nextNextNextNextNextNextNextDraw.red === 'string') {
          const numbers = nextNextNextNextNextNextNextDraw.red.match(/\d+/g);
          if (numbers) {
            nextNextNextNextNextNextNextFrontNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
      ...
      recommendedFrontBuyNumbers.forEach(buyNum => {
        if (nextNextNextNextNextNextNextFrontNumbers.includes(buyNum)) {
      ...
      backtestResults.push({
        ...
        nextNextNextNextNextNextNextFrontNumbers: nextNextNextNextNextNextNextFrontNumbers, // 下下下下下下下期实际开奖前区
...
/**
 * 倒数7期两球组合前区买号回测接口
 * @route GET /dao_shu_7_qi_mai_hao_hui_ce
 * @group 数据分析 - 买号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await dao_shu_7_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method);
