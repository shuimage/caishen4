...
// 接口功能: 分析倒数第7期前区两球组合在下下下下下下下期的出现情况
// 返回倒数第7期前区两球组合的统计数据
...
async function getSeventhLastFrontZoneAnalysis(period) {
  try {
    ...
    console.log('红球数据:', seventhLastDraw.red);
    
    // 解析前区号码（从red字段中获取）
    let frontNumbers = [];
    try {
      // 使用processBalls函数处理号码，确保格式一致
      const processedBalls = processBalls(seventhLastDraw.red);
      // 转换回数字数组，并过滤掉非数字值
      frontNumbers = processedBalls.map(num => parseInt(num)).filter(num => !isNaN(num));
      // 确保frontNumbers至少有一些号码，否则记录错误
      if (!frontNumbers.length) {
        console.error('Failed to extract valid front numbers from:', seventhLastDraw.red);
      }
    } catch (e) {
    ...
    // 创建bian_hao到数据的映射，用于快速查找下下下下下下下期数据
    ...
    // 使用bian_hao字段计算下下下下下下下期，而不是依赖索引
    for (let i = 0; i < historyData.length; i++) { // 从i=0开始，不跳过任何记录
      ...
      const nextNextNextNextNextNextNextNumericBianHao = currentNumericBianHao + 7;
      
      // 查找下下下下下下下期数据
      const nextNextNextNextNextNextNextDraw = numericBianHaoToDataMap.get(nextNextNextNextNextNextNextNumericBianHao);
      ...
      currentFrontCombinations.forEach(comb => {
        if (frontCombinations.includes(comb)) {
          ...
          try {
            // 使用processBalls函数处理号码，确保格式一致
            const processedBalls = processBalls(nextNextNextNextNextNextNextDraw.red);
            // 存储两位格式的号码，用于统计
            nextNextNextNextNextNextNextNumbers = processedBalls;
          } catch (e) {
            console.error('Error extracting next next next next next next next draw red numbers:', e);
          }
          ...
          frontCombinationStats[comb].occurrenceDetails.push({
            ...
            nextNextNextNextNextNextNextFrontNumbers: nextNextNextNextNextNextNextNumbers
          });
    ...
    const frontNumberCounts = {}; // 前区1-35：记录每个号码在组合出现时下下下下下下下期的出现次数
    for (let i = 1; i <= 35; i++) frontNumberCounts[i] = 0;
    
    // 统计组合出现时下下下下下下下期号码的出现次数（基于前端表格显示的数据维度）
    console.log('开始统计组合出现时下下下下下下下期号码的出现次数');
    
    // 遍历所有组合的统计数据
    Object.values(frontCombinationStats).forEach(combination => {
      // 获取当前组合对应的下下下下下下下期号码列表
    ...
    // 输出号码统计结果（基于组合出现时下下下下下下下期的维度）
    console.log('组合出现时下下下下下下下期号码出现次数统计结果:', frontNumberCounts);
    ...
    console.log('前区推荐杀号计算完成(基于组合出现时下下下下下下下期维度):', recommendedFrontKillNumbers);
    ...
    console.error('倒数第7期前区两球组合分析出错:', error);
...
/**
 * 倒数第7期前区两球组合分析接口
 * @route GET /dao_shu_7_qi_liang_qiu_zu_he
 * @group 数据分析 - 倒数第7期前区两球组合分析相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getSeventhLastFrontZoneAnalysis(processedPeriod);
