// 倒数7期两球组合详情服务器 - 支持查询倒数7期两球组合在下下下下下下下期的出现情况
...
router.post('/', async (req, res) => {
  try {
    ...
    // 直接查询倒数第7期的开奖数据（基于issue排序，取最新的第7条）
    let daoShu7QiData = await query(
      'SELECT * FROM lottery_results ORDER BY issue DESC LIMIT 6, 1'
    );
    
    if (!daoShu7QiData || daoShu7QiData.length === 0) {
      return res.status(404).json({ code: 404, message: '未找到倒数7期的开奖数据' });
    }
    
    const daoShu7QiResult = daoShu7QiData[0];
    const daoShu7QiPeriod = daoShu7QiResult.issue;
    console.log('查询倒数7期:', daoShu7QiPeriod);
    
    // 提取倒数7期的开奖号码
    const drawField = type === 'front' ? 'red' : 'blue';
    const drawNumbers = processBalls(daoShu7QiResult[drawField]);
    
    console.log('倒数7期开奖号码:', drawNumbers);
    ...
    // 确保daoShu7QiResult和id存在
    if (!daoShu7QiResult || typeof daoShu7QiResult.id === 'undefined') {
      console.error('倒数7期数据缺少id字段:', daoShu7QiResult);
      return res.status(500).json({ code: 500, message: '获取倒数7期数据失败，缺少id字段' });
    }
    ...
    // 创建bian_hao到数据的映射，方便快速查找下下下下下下下期数据
    const bianHaoToDataMap = new Map();
    // 创建数字bian_hao到数据的映射，用于计算下下下下下下下期
    ...
    rawResults.forEach(row => {
      try {
        ...
        if (isMatch) {
          ...
          // 计算下下下下下下下期的数字bian_hao（当前期+7）
          const nextNextNextNextNextNextNumericBianHao = currentNumericBianHao + 7;
          console.log('下下下下下下下期bian_hao数字部分:', nextNextNextNextNextNextNumericBianHao);
          
          // 查找下下下下下下下期数据
          const nextNextNextNextNextNextData = numericBianHaoToDataMap.get(nextNextNextNextNextNextNumericBianHao);
          
          if (nextNextNextNextNextNextData) {
            console.log('找到下下下下下下下期数据:', nextNextNextNextNextNextData.issue);
            const nextNextNextNextNextNextDrawNumbers = processBalls(nextNextNextNextNextNextData.draw_info);
            ...
            results.push({
              ...
              next_next_next_next_next_next_period: nextNextNextNextNextNextData.issue,
              next_next_next_next_next_next_draw_info: nextNextNextNextNextNextDrawNumbers.join(' ')
            });
          } else {
            console.log('未找到下下下下下下下期数据:', nextNextNextNextNextNextNumericBianHao);
          }
        }
      } catch (error) {
    ...
    if (target_ball !== null && target_ball !== undefined && target_ball !== '') {
      ...
      results = results.filter(result => {
        const nextDrawNumbers = processBalls(result.next_next_next_next_next_next_draw_info);
        console.log('检查记录:', result.period, '下下下下下下下期:', result.next_next_next_next_next_next_period, '开奖号码:', nextDrawNumbers, '是否包含目标球:', nextDrawNumbers.includes(targetBallStr));
