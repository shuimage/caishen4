...
// 计算后区号码的推荐买号
function calculateBackRecommendations(last8Periods) {
  const numberCounts = {};
  
  // 统计每个号码在8期内的出现次数
  last8Periods.forEach(item => {
...
router.get('/', async (req, res) => {
  try {
    ...
    // 执行回测
    for (let i = 8; i < results.length; i += 1) {
      ...
      // 获取当前期之前的8期数据用于分析
      const last8Periods = results.slice(i + 1, i + 1 + 8);
      
      if (last8Periods.length < 8) {
        break; // 数据不足
      }
      
      // 计算推荐买号
      const recommendedNumbers = calculateBackRecommendations(last8Periods);
      ...
      // 获取下下下下下下下下期的开奖数据（索引为 i - 8）
      const next7thIndex = i - 8;
      if (next7thIndex < 0) {
        continue; // 没有足够的历史数据
      }
      
      const next7thPeriod = results[next7thIndex];
      
      // 计算正确买号数和错误买号数
      const backCorrectBuy = next7thPeriod.lastZoneNumbers.includes(selectedBuyNumber) ? 1 : 0;
      ...
      backtestResults.push({
        ...
        nextNextNextNextNextNextNextIssue: next7thPeriod.period,
        nextNextNextNextNextNextNextBackNumbers: next7thPeriod.lastZoneNumbers,
        backCorrectBuy,
        backWrongBuy
      });
    }
