...
// 接口功能: 分析倒数第8期后区两球组合在下下下下下下下下期的出现情况
// 返回倒数第8期后区两球组合的统计数据
...
async function getEighthLastBackZoneAnalysis(period) {
  try {
    ...
    console.log('蓝球数据:', eighthLastDraw.blue);
    
    // 解析后区号码（从blue字段中获取）
    let backNumbers = [];
    try {
      if (Array.isArray(eighthLastDraw.blue)) {
        // 如果已经是数组，直接使用
        backNumbers = eighthLastDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
      } else if (typeof eighthLastDraw.blue === 'string') {
        // 从字符串中提取数字
        const numbers = eighthLastDraw.blue.match(/\d+/g);
        if (numbers) {
      ...
      if (!backNumbers.length) {
        console.error('Failed to extract back numbers from:', eighthLastDraw.blue);
      }
    } catch (e) {
    ...
    for (let i = 8; i < historyData.length; i++) {
      ...
      currentBackCombinations.forEach(comb => {
        if (backCombinations.includes(comb)) {
          ...
          try {
            if (Array.isArray(nextNextNextNextNextNextNextNextDraw.blue)) {
              nextNextNextNextNextNextNextNextNumbers = nextNextNextNextNextNextNextNextDraw.blue.map(num => typeof num === 'string' ? parseInt(num) : num);
            } else if (typeof nextNextNextNextNextNextNextNextDraw.blue === 'string') {
              nextNextNextNextNextNextNextNextNumbers = nextNextNextNextNextNextNextNextDraw.blue.match(/\d+/g)?.map(num => parseInt(num)) || [];
            }
          } catch (e) {
          ...
          backCombinationStats[comb].occurrenceDetails.push({
            ...
            nextNextNextNextNextNextNextNextBackNumbers: nextNextNextNextNextNextNextNextNumbers
          });
    ...
    console.error('倒数第8期后区两球组合分析出错:', error);
...
/**
 * 倒数第8期后区两球组合分析接口
 * @route GET /dao_shu_8_qi_hou_qu_zu_he
 * @group 数据分析 - 倒数第8期后区两球组合分析相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getEighthLastBackZoneAnalysis(processedPeriod);
