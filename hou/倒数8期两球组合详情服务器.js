// 倒数8期两球组合详情服务器 - 支持查询倒数8期两球组合在下下下下下下下下期的出现情况
...
router.post('/', async (req, res) => {
  try {
    ...
    // 直接查询倒数第8期的开奖数据（基于issue排序，取最新的第8条）
    let daoShu8QiData = await query(
      'SELECT * FROM lottery_results ORDER BY issue DESC LIMIT 7, 1'
    );
    
    if (!daoShu8QiData || daoShu8QiData.length === 0) {
      return res.status(404).json({ code: 404, message: '未找到倒数8期的开奖数据' });
    }
    
    const daoShu8QiResult = daoShu8QiData[0];
    const daoShu8QiPeriod = daoShu8QiResult.issue;
    console.log('查询倒数8期:', daoShu8QiPeriod);
    
    // 提取倒数8期的开奖号码
    const drawField = type === 'front' ? 'red' : 'blue';
    const drawNumbers = processBalls(daoShu8QiResult[drawField]);
    
    console.log('倒数8期开奖号码:', drawNumbers);
    ...
    // 确保daoShu8QiResult和id存在
    if (!daoShu8QiResult || typeof daoShu8QiResult.id === 'undefined') {
      console.error('倒数8期数据缺少id字段:', daoShu8QiResult);
      return res.status(500).json({ code: 500, message: '获取倒数8期数据失败，缺少id字段' });
    }
    ...
    // 创建bian_hao到数据的映射，方便快速查找下下下下下下下下期数据
    const bianHaoToDataMap = new Map();
    // 创建数字bian_hao到数据的映射，用于计算下下下下下下下下期
    ...
    rawResults.forEach(row => {
      try {
        ...
        if (isMatch) {
          ...
          // 计算下下下下下下下下期的数字bian_hao（当前期+8）
          const nextNextNextNextNextNextNextNumericBianHao = currentNumericBianHao + 8;
          console.log('下下下下下下下下期bian_hao数字部分:', nextNextNextNextNextNextNextNumericBianHao);
          
          // 查找下下下下下下下下期数据
          const nextNextNextNextNextNextNextData = numericBianHaoToDataMap.get(nextNextNextNextNextNextNextNumericBianHao);
          
          if (nextNextNextNextNextNextNextData) {
            console.log('找到下下下下下下下下期数据:', nextNextNextNextNextNextNextData.issue);
            const nextNextNextNextNextNextNextDrawNumbers = processBalls(nextNextNextNextNextNextNextData.draw_info);
            
            // 直接添加结果，后续统一处理目标球
            results.push({
              id: row.id,
              period: row.issue,
              draw_info: currentDrawNumbers.join(' '),
              next_next_next_next_next_next_next_period: nextNextNextNextNextNextNextData.issue,
              next_next_next_next_next_next_next_draw_info: nextNextNextNextNextNextNextDrawNumbers.join(' ')
            });
          } else {
            console.log('未找到下下下下下下下下期数据:', nextNextNextNextNextNextNextNumericBianHao);
          }
        }
      } catch (error) {
    ...
    // 如果有目标球，检查目标球是否在下下下下下下下下期出现
    if (target_ball !== null && target_ball !== undefined && target_ball !== '') {
      console.log('开始目标球过滤，目标球:', target_ball);
      const targetBallStr = String(target_ball).padStart(2, '0');
      console.log('目标球转换后:', targetBallStr);
      console.log('过滤前结果数量:', results.length);
      
      results = results.filter(result => {
        const nextDrawNumbers = processBalls(result.next_next_next_next_next_next_next_draw_info);
        console.log('检查记录:', result.period, '下下下下下下下下期:', result.next_next_next_next_next_next_next_period, '开奖号码:', nextDrawNumbers, '是否包含目标球:', nextDrawNumbers.includes(targetBallStr));
        return nextDrawNumbers.includes(targetBallStr);
      });
      
      console.log('过滤后结果数量:', results.length);
    }
