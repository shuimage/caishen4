...
// 接口功能: 回测倒数9期两球组合前区买号结果，比较推荐买号与真实的开奖结果
// 返回每一期的回测数据，包括期号、推荐买号、下下下下下下下下下期开奖号、正确买号和错误买号数量
 ...
 */
async function dao_shu_9_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most') {
  try {
    // 解析参数
    ...
    if (parsedBacktestPeriod === 'all') {
      ...
      const historySql = `
        ...
        FROM lottery_results 
        ORDER BY issue DESC 
        LIMIT ${limit + 9}  -- 多获取9期，用于回测比较（需要下下下下下下下下下期数据）
      `;
      historyData = await query(historySql);
    }
    ...
    // 对每一期进行回测（跳过最新9期，因为没有下下下下下下下下下期数据）
    for (let i = 9; i < historyData.length; i++) {
      const currentDraw = historyData[i];  // 当前回测期
      const nextNextNextNextNextNextNextNextNextDraw = historyData[i - 9]; // 下下下下下下下下下期开奖结果（用于比较）
      ...
      // 遍历统计历史数据，查找组合出现的位置，并记录下下下下下下下下下期的号码
      for (let j = 9; j < statsHistoryData.length; j++) {
        const statsCurrentDraw = statsHistoryData[j];
        const statsNextNextNextNextNextNextNextNextNextDraw = statsHistoryData[j - 9];
        ...
        statsCurrentFrontCombinations.forEach(comb => {
          if (frontCombinations.includes(comb)) {
            // 获取下下下下下下下下下期的前区号码
            let nextNextNextNextNextNextNextNextNextNumbers = [];
            try {
              if (Array.isArray(statsNextNextNextNextNextNextNextNextNextDraw.red)) {
                nextNextNextNextNextNextNextNextNextNumbers = statsNextNextNextNextNextNextNextNextNextDraw.red.map(num => typeof num === 'string' ? parseInt(num) : num);
              } else if (typeof statsNextNextNextNextNextNextNextNextNextDraw.red === 'string') {
                nextNextNextNextNextNextNextNextNextNumbers = statsNextNextNextNextNextNextNextNextNextDraw.red.match(/\d+/g)?.map(num => parseInt(num)) || [];
              }
            } catch (e) {
              console.error('解析统计下下下下下下下下下期前区号码失败:', e);
              return;
            }
            
            // 更新号码计数
            nextNextNextNextNextNextNextNextNextNumbers.forEach(num => {
      ...
      // 解析下下下下下下下下下期的前区号码
      let nextNextNextNextNextNextNextNextNextFrontNumbers = [];
      try {
        if (Array.isArray(nextNextNextNextNextNextNextNextNextDraw.red)) {
          nextNextNextNextNextNextNextNextNextFrontNumbers = nextNextNextNextNextNextNextNextNextDraw.red.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof nextNextNextNextNextNextNextNextNextDraw.red === 'string') {
          const numbers = nextNextNextNextNextNextNextNextNextDraw.red.match(/\d+/g);
          if (numbers) {
            nextNextNextNextNextNextNextNextNextFrontNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
        console.error('解析下下下下下下下下下期前区号码失败:', e);
        continue;
      }
      ...
      recommendedFrontBuyNumbers.forEach(buyNum => {
        if (nextNextNextNextNextNextNextNextNextFrontNumbers.includes(buyNum)) {
          // 正确买号：推荐买号出现在下下下下下下下下下期开奖号中
          frontCorrectBuy++;
        } else {
          // 错误买号：推荐买号没有出现在下下下下下下下下下期开奖号中
          frontWrongBuy++;
        }
      });
      ...
      backtestResults.push({
        ...
        nextNextNextNextNextNextNextNextNextIssue: nextNextNextNextNextNextNextNextNextDraw.issue, // 下下下下下下下下下期开奖期号
        nextNextNextNextNextNextNextNextNextFrontNumbers: nextNextNextNextNextNextNextNextNextFrontNumbers, // 下下下下下下下下下期实际开奖前区
...
/**
 * 倒数9期两球组合前区买号回测接口
 * @route GET /dao_shu_9_qi_mai_hao_hui_ce
 * @group 数据分析 - 买号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await dao_shu_9_qi_mai_hao_hui_ce(backtest_period, stats_period, backtest_method);
