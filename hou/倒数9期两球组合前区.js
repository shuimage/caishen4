...
// 接口功能: 分析倒数第9期前区两球组合在下下下下下下下下下期的出现情况
// 返回倒数第9期前区两球组合的统计数据
...
async function getNinthLastFrontZoneAnalysis(period) {
  try {
    ...
    console.log('红球数据:', ninthLastDraw.red);
    
    // 解析前区号码（从red字段中获取）
    let frontNumbers = [];
    try {
      // 使用processBalls函数处理号码，确保格式一致
      const processedBalls = processBalls(ninthLastDraw.red);
      // 转换回数字数组，并过滤掉非数字值
      frontNumbers = processedBalls.map(num => parseInt(num)).filter(num => !isNaN(num));
      // 确保frontNumbers至少有一些号码，否则记录错误
      if (!frontNumbers.length) {
        console.error('Failed to extract valid front numbers from:', ninthLastDraw.red);
      }
    } catch (e) {
    ...
    frontCombinations.length = 0;
    frontCombinations.push(...uniqueFrontCombinations);
    ...
    for (const currentDraw of historyData) { // 遍历所有记录
      ...
      // 检查前区是否包含我们关心的组合
      for (const comb of currentFrontCombinations) {
          ...
          if (currentDraw.bian_hao) {
            ...
            if (!isNaN(number)) {
              ...
              const nextNextNextNextNextNextNextNextData = await query(
                'SELECT issue, red, draw_date FROM lottery_results WHERE bian_hao = ?',
              ...
              if (nextNextNextNextNextNextNextNextData && nextNextNextNextNextNextNextNextData.length > 0) {
                ...
                frontCombinationStats[comb].occurrenceDetails.push({
                  ...
                  nextNextNextNextNextNextNextNextNextFrontNumbers: nextNextNextNextNextNextNextNextNumbers
                });
    ...
    const frontNumberCounts = {}; // 前区1-35：记录每个号码在组合出现时下下下下下下下下下期的出现次数
    ...
    console.log('组合出现时下下下下下下下下下期号码出现次数统计结果:', frontNumberCounts);
    ...
    console.log('前区推荐杀号计算完成(基于组合出现时下下下下下下下下下期维度):', recommendedFrontKillNumbers);
    ...
    console.error('倒数第9期前区两球组合分析出错:', error);
...
/**
 * 倒数第9期前区两球组合分析接口
 * @route GET /dao_shu_9_qi_liang_qiu_zu_he
 * @group 数据分析 - 倒数第9期前区两球组合分析相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getNinthLastFrontZoneAnalysis(processedPeriod);
