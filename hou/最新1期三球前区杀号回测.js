...
// 接口功能: 回测统计范围内的三球组合前区推荐杀号，比较推荐杀号与实际开奖结果
...
/**
 * 计算推荐杀号（与 san_qiu_fen_xi.html 页相同的算法）
 * @param {Array} combinations - 三球组合数组
 ...
 */
async function getSanQiuKillNumberBacktest(backtest_period, stats_period) {
  try {
    // 验证参数
    ...
    console.error('三球组合前区杀号回测出错:', error);
...
/**
 * 三球组合前区杀号回测接口
 * @route GET /san_qiu_sha_hao_hui_ce
 * @group 数据分析 - 三球组合前区杀号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getSanQiuKillNumberBacktest(backtest_period, stats_period);
