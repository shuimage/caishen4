...
// 返回每一期的回测数据，包括期号、推荐杀号、下一期开奖号、正确杀号和错误杀号数量
 ...
 */
async function getKillNumberBacktest(backtest_period, stats_period) {
  try {
    // 验证参数
    ...
    for (let i = 1; i < historyData.length; i++) {
      ...
      const recommendedBackKillNumbers = calculateKillNumbers(backCombinations, backCombinationStats, 1, 12);
      ...
      backtestResults.push({
        currentIssue: currentDraw.issue, // 当前期号
        frontKillNumbers: recommendedFrontKillNumbers, // 前区推荐杀号
        backKillNumbers: recommendedBackKillNumbers, // 后区推荐杀号
        nextIssue: nextDraw.issue, // 下一期开奖期号
        nextFrontNumbers: nextFrontNumbers, // 下一期实际开奖前区
        nextBackNumbers: nextBackNumbers, // 下一期实际开奖后区
...
/**
 * 杀号回测接口
 * @route GET /huo_qu_sha_hao_hui_ce
 * @group 数据分析 - 杀号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getKillNumberBacktest(backtest_period, stats_period);
