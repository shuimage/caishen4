 ...
 */
async function getJinLiangQiKillNumberBacktest(backtest_period, stats_period) {
  try {
    // 验证参数
    ...
    for (let i = 3; i < historyData.length; i++) {
      ...
      // 解析近两期的前区号码
      let basePeriod1FrontNumbers = [];
      let basePeriod2FrontNumbers = [];
      
      try {
        // 解析第N-1期前区号码
        if (Array.isArray(basePeriod1.red)) {
          basePeriod1FrontNumbers = basePeriod1.red.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof basePeriod1.red === 'string') {
          const numbers = basePeriod1.red.match(/\d+/g);
          if (numbers) {
            basePeriod1FrontNumbers = numbers.map(num => parseInt(num));
          }
        }
        
        // 解析第N-2期前区号码
        if (Array.isArray(basePeriod2.red)) {
          basePeriod2FrontNumbers = basePeriod2.red.map(num => typeof num === 'string' ? parseInt(num) : num);
        } else if (typeof basePeriod2.red === 'string') {
          const numbers = basePeriod2.red.match(/\d+/g);
          if (numbers) {
            basePeriod2FrontNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
        console.error('解析近两期前区号码失败:', e);
        continue;
      }
      ...
      // 生成第N-1期的两球组合
      for (let j = 0; j < basePeriod1FrontNumbers.length; j++) {
        for (let k = j + 1; k < basePeriod1FrontNumbers.length; k++) {
          // 确保小球在前，大球在后，保持一致性
          const ball1 = Math.min(basePeriod1FrontNumbers[j], basePeriod1FrontNumbers[k]);
          const ball2 = Math.max(basePeriod1FrontNumbers[j], basePeriod1FrontNumbers[k]);
          frontCombinations.push(`${ball1}-${ball2}`);
        }
      }
      
      // 生成第N-2期的两球组合
      for (let j = 0; j < basePeriod2FrontNumbers.length; j++) {
        for (let k = j + 1; k < basePeriod2FrontNumbers.length; k++) {
          // 确保小球在前，大球在后，保持一致性
          const ball1 = Math.min(basePeriod2FrontNumbers[j], basePeriod2FrontNumbers[k]);
          const ball2 = Math.max(basePeriod2FrontNumbers[j], basePeriod2FrontNumbers[k]);
          frontCombinations.push(`${ball1}-${ball2}`);
        }
      }
      
      // 去重组合
      const uniqueFrontCombinations = [...new Set(frontCombinations)];
      ...
      uniqueFrontCombinations.forEach(combination => {
      ...
      if (statsHistoryData.length >= 2) {
        ...
        statsCurrentFrontCombinations.forEach(comb => {
          if (uniqueFrontCombinations.includes(comb)) {
            // 获取下一期的前区号码
            let nextNumbers = [];
            try {
      ...
      // 计算当前期的推荐杀号
      const recommendedFrontKillNumbers = calculateKillNumbers(uniqueFrontCombinations, frontCombinationStats, 1, 35);
      ...
      try {
        if (Array.isArray(nextDraw.red)) {
          ...
          const numbers = nextDraw.red.match(/\d+/g);
          if (numbers) {
            nextFrontNumbers = numbers.map(num => parseInt(num));
          }
        }
      } catch (e) {
        console.error('解析下一期前区号码失败:', e);
        continue;
      }
      ...
      recommendedFrontKillNumbers.forEach(killNum => {
        if (!nextFrontNumbers.includes(killNum)) {
      ...
      backtestResults.push({
        ...
        recommendedKills: recommendedFrontKillNumbers, // 前区推荐杀号
        nextPeriod: nextDraw.issue, // 下一期开奖期号
        nextDrawNumbers: nextFrontNumbers, // 下一期实际开奖前区
        killResults: killResults, // 杀号结果详情
        correctKills: correctKills, // 前区正确杀号
        wrongKills: wrongKills, // 前区错误杀号
...
/**
 * 近两期两球组合前区杀号回测接口
 * @route POST /jin_liang_qi_qian_qu_sha_hao_hui_ce
 * @group 数据分析 - 杀号回测相关接口
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await getJinLiangQiKillNumberBacktest(backtest_period, stats_range);
