...
// 接口功能: 回测近两期两球组合后区买号结果，比较推荐买号与真实的开奖结果
 ...
 */
async function huo_qu_jin_liang_qi_hou_qu_mai_hao_hui_ce(backtest_period, stats_period, backtest_method = 'most') {
  try {
    // 验证参数
...
/**
 * 近两期两球组合后区买号回测接口
 * @route GET /jin_liang_qi_hou_qu_mai_hao_hui_ce
 * @group 数据分析 - 买号回测相关接口
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await huo_qu_jin_liang_qi_hou_qu_mai_hao_hui_ce(backtest_period, stats_period, backtest_method);
