...
router.post('/', async (req, res) => {
  try {
    ...
    console.log('收到近两期后区两球组合统计请求，请求参数:', { stats_range });
    ...
    // 分别获取两期的后区号码
    const backNumbers1 = processBalls(latestDraws[0].blue); // 第1期（最新一期）
    const backNumbers2 = processBalls(latestDraws[1].blue); // 第2期
    
    console.log('第1期后区号码:', backNumbers1);
    console.log('第2期后区号码:', backNumbers2);
    
    // 只生成两期间的组合，第2期的号在前，第1期的号在后
    const combinations = [];
    for (const num2 of backNumbers2) {
      for (const num1 of backNumbers1) {
    ...
    for (const combo of combinations) {
      ...
      for (let i = 0; i < processedHistory.length - 2; i++) {
        ...
        if (checkBallInDraw(nextDraw.blue, ball1) && checkBallInDraw(currentDraw.blue, ball2)) {
          // 找到连续两期，统计下下期（第3期）的球号
          matchCount++;
          
          if (nextNextDraw) {
            nextNextDraw.blue.forEach(ball => {
