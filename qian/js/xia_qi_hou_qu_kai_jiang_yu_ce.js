...
window.addEventListener('load', async function() {
  ...
  // 回测期数全局变量
  let backtestPeriod = '20';
  
  // 获取配置的回测期数
  function getBacktestPeriod() {
    const input = document.getElementById('backtestPeriod');
    if (input) {
  ...
  async function dao_shu_2_qi_mai_hao_hui_ce(backtestPeriod = '50', statsPeriod = '100', backtestMethod = 'most') {
    try {
      const response = await fetch(`http://localhost:18889/dao_shu_2_qi_mai_hao_hui_ce_hou?backtest_period=${backtestPeriod}&stats_period=${statsPeriod}&backtest_method=${backtestMethod}`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const result = await response.json();
  ...
  // 获取后区推荐买号 - 使用与sha_hao_fen_xi.html相同的算法
  ...
  // 测试单个统计期值的平均正确率
  async function testStatsPeriod(backtestPeriod, statsPeriod, backtestMethod, isSecondLast = false, isThirdLast = false, isFourthLast = false, isFifthLast = false) {
    try {
      let backtestData;
      if (isFifthLast) {
        ...
        backtestData = await huo_qu_sha_hao_hou_mai_hao_hui_ce(backtestPeriod, statsPeriod, backtestMethod);
      }
      
      if (!backtestData || !backtestData.backtestResults || backtestData.backtestResults.length === 0) {
        return { statsPeriod, accuracy: isSecondLast ? 100 : 0 };
      }
      ...
      const averageCorrectRate = totalPeriods > 0 ? 
        ((successBuyCount / totalPeriods) * 100) : (isSecondLast ? 100 : 0);
  ...
  // 获取倒数2期的下下期期号 - 使用与dao_shu_2_qi_hou_ping_jun_lv_zui_gao_detail.html相同的算法
  ...
  // 获取倒数3期的下下下期期号 - 使用与dao_shu_3_qi_hou_ping_jun_lv_zui_gao_detail.html相同的算法
  async function getLatestPeriodThirdLast() {
    try {
      ...
      if (result.success && result.latestDraw) {
        ...
        const lastPeriodNum = parseInt(lastPeriod.replace('期', ''));
        return (lastPeriodNum + 3) + '期';
      }
      return '最新期';
    } catch (error) {
      console.error('获取最新期号失败', error);
  ...
  // 获取倒数4期的下下下下期期号 - 使用与dao_shu_4_qi_hou_ping_jun_lv_zui_gao_detail.html相同的算法
  async function getLatestPeriodFourthLast() {
    try {
      // 调用sql_zui_xin_yi_qi接口获取最新一期开奖信息
      const response = await fetch('http://localhost:18889/sql_zui_xin_yi_qi');
      const result = await response.json();
      if (result.success && result.data && result.data.length > 0) {
        const latestPeriod = result.data[0].period || result.data[0].issue;
        const latestPeriodNum = parseInt(latestPeriod);
        return (latestPeriodNum + 4) + '期'; // 下下下下期
      }
      return '最新期';
    } catch (error) {
      console.error('获取最新期号失败', error);
  ...
  // 获取倒数5期的下下下下下期期号 - 使用与dao_shu_5_qi_hou_ping_jun_lv_zui_gao_detail.html相同的算法
  async function getLatestPeriodFifthLast() {
    try {
      // 调用sql_zui_xin_yi_qi接口获取最新一期开奖信息
      const response = await fetch('http://localhost:18889/sql_zui_xin_yi_qi');
      const result = await response.json();
      if (result.success && result.data && result.data.length > 0) {
        const latestPeriod = result.data[0].period || result.data[0].issue;
        const latestPeriodNum = parseInt(latestPeriod);
        return (latestPeriodNum + 5) + '期'; // 下下下下下期
      }
      return '最新期';
    } catch (error) {
      console.error('获取最新期号失败', error);
  ...
  async function bao_cun_hui_ce_jie_guo_quan_bu() {
    ...
    if (hasAnyResults) {
      // 获取当前回测期数配置
      const currentBacktestPeriod = getBacktestPeriod();
      
      // 保存到localStorage
      localStorage.setItem('backtestResults', JSON.stringify(backtestResults));
      localStorage.setItem('backtestPeriod', currentBacktestPeriod);
      ...
      try {
        ...
        const response = await fetch('http://localhost:18889/bao_cun_hui_ce_jie_guo', {
          ...
          body: JSON.stringify({
            ...
            backtest_period: currentBacktestPeriod
          })
        });
  ...
  async function jia_zai_hui_ce_jie_guo() {
    try {
      ...
      if (result.success && result.data) {
        ...
        const backtestPeriodFromDB = result.backtest_period;
        
        // 保存到localStorage作为备份
        localStorage.setItem('backtestResults', JSON.stringify(backtestResults));
        if (backtestPeriodFromDB) {
          localStorage.setItem('backtestPeriod', backtestPeriodFromDB);
          // 更新回测期数输入框
          const backtestPeriodInput = document.getElementById('backtestPeriod');
          if (backtestPeriodInput) {
            backtestPeriodInput.value = backtestPeriodFromDB;
            backtestPeriod = backtestPeriodFromDB;
          }
        }
        ...
        if (savedResults) {
          ...
          const savedBacktestPeriod = localStorage.getItem('backtestPeriod');
          if (savedBacktestPeriod) {
            const backtestPeriodInput = document.getElementById('backtestPeriod');
            if (backtestPeriodInput) {
              backtestPeriodInput.value = savedBacktestPeriod;
              backtestPeriod = savedBacktestPeriod;
            }
          }
          await updateUIWithBacktestResults(backtestResults);
          alert('回测结果已从本地加载');
  ...
  // 渲染详情数据 - 最新一期两球组合后区
  async function renderDetailDataNewestBack(allResults, nextPeriod, backtestPeriod) {
    ...
    try {
      ...
      // 更新后区推荐买号合集
      const buyNumbersContent = document.getElementById('buyNumbersCollectionContentNewestBack');
        ...
        if (allBuyNumbers.length > 0) {
          ...
          buyNumbersContent.innerHTML = sortedNumbers.map(item => {
            ...
            return `<div style="display: flex; align-items: center; gap: 5px;">
              <div class="blue-ball">${item.number}</div>
  ...
  async function performSearchNewestBack() {
    ...
    try {
      ...
      const backtestPeriod = getBacktestPeriod(); // 使用配置的回测期数
      ...
      // 获取最后一期期号
      const lastPeriod = await getLastPeriod();
      const nextPeriod = lastPeriod + '期'; // 使用最近期的期号作为当前期号
      ...
      if (isBacktestStopped) {
        ...
        if (isBacktestStopped) {
          ...
          // 渲染详情数据，传递过滤后的结果、下一期期号和回测期数
          await renderDetailDataNewestBack(filteredResults, nextPeriod, backtestPeriod);
  ...
  // 渲染详情数据 - 倒数2期两球组合后区
  async function renderDetailDataSecondLastBack(allResults, nextPeriod, backtestPeriod) {
    ...
    try {
      ...
      // 更新后区推荐买号合集
      const buyNumbersContent = document.getElementById('buyNumbersCollectionContentSecondLastBack');
      if (buyNumbersContent) {
        if (allBuyNumbers.length > 0) {
          // 按平均正确率累加进行排序，同一个推荐买号的球，回测方法相同时，只保留平均正确率大的
          const numberAccuracyMap = new Map();
  ...
  // 渲染详情数据 - 倒数3期两球组合后区
  async function renderDetailDataThirdLastBack(allResults, nextPeriod, backtestPeriod) {
    ...
    try {
      ...
      // 更新后区推荐买号合集
      const buyNumbersContent = document.getElementById('buyNumbersCollectionContentThirdLastBack');
      if (buyNumbersContent) {
        if (allBuyNumbers.length > 0) {
          // 按平均正确率累加进行排序，同一个推荐买号的球，回测方法相同时，只保留平均正确率大的
          const numberAccuracyMap = new Map();
  ...
  // 渲染详情数据 - 倒数4期两球组合后区
  async function renderDetailDataFourthLastBack(allResults, nextPeriod, backtestPeriod) {
    ...
    try {
      ...
      // 更新后区推荐买号合集
      const buyNumbersContent = document.getElementById('buyNumbersCollectionContentFourthLastBack');
      if (buyNumbersContent) {
        if (allBuyNumbers.length > 0) {
          // 按平均正确率累加进行排序，同一个推荐买号的球，回测方法相同时，只保留平均正确率大的
          const numberAccuracyMap = new Map();
  ...
  // 渲染详情数据 - 倒数5期两球组合后区
  async function renderDetailDataFifthLastBack(allResults, nextPeriod, backtestPeriod) {
    ...
    try {
      ...
      // 更新后区推荐买号合集
      const buyNumbersContent = document.getElementById('buyNumbersCollectionContentFifthLastBack');
      if (buyNumbersContent) {
        if (allBuyNumbers.length > 0) {
          // 按平均正确率累加进行排序，同一个推荐买号的球，回测方法相同时，只保留平均正确率大的
          const numberAccuracyMap = new Map();
      ...
      // 生成后区推荐买号总集
      await generateBackBuyNumbersTotal();
    }
  }
  
  // 生成后区推荐买号总集
  async function generateBackBuyNumbersTotal() {
    try {
      // 收集所有卡片的后区推荐买号数据
      const totalNumberAccuracyMap = new Map();
      
      // 遍历所有卡片的推荐买号合集
      const collectionContents = [
        'buyNumbersCollectionContentNewestBack',
        'buyNumbersCollectionContentSecondLastBack',
        'buyNumbersCollectionContentThirdLastBack',
        'buyNumbersCollectionContentFourthLastBack',
        'buyNumbersCollectionContentFifthLastBack'
      ];
      
      collectionContents.forEach(contentId => {
        const content = document.getElementById(contentId);
        if (content) {
          const numberElements = content.querySelectorAll('.blue-ball');
          numberElements.forEach(element => {
            const number = parseInt(element.textContent);
            const accuracyElement = element.nextElementSibling;
            if (accuracyElement) {
              const accuracyText = accuracyElement.textContent;
              const accuracy = parseFloat(accuracyText.replace('%', ''));
              
              if (!isNaN(number) && !isNaN(accuracy)) {
                const numStr = number.toString();
                if (totalNumberAccuracyMap.has(numStr)) {
                  const existingData = totalNumberAccuracyMap.get(numStr);
                  const newAccuracy = existingData.accuracy + accuracy;
                  totalNumberAccuracyMap.set(numStr, { accuracy: newAccuracy, number: number });
                } else {
                  totalNumberAccuracyMap.set(numStr, { accuracy, number: number });
                }
              }
            }
          });
      ...
      // 转换为数组并按平均正确率降序排序
      const sortedNumbers = Array.from(totalNumberAccuracyMap.values())
        .sort((a, b) => b.accuracy - a.accuracy);
      
      // 更新后区推荐买号总集
      const totalRankingContainer = document.getElementById('buyNumbersTotalRankingFifthLastBack');
      if (totalRankingContainer) {
        if (sortedNumbers.length > 0) {
          // 生成排名列表的HTML
          totalRankingContainer.innerHTML = sortedNumbers.map((item, index) => {
            const accuracyText = item.accuracy.toFixed(3);
            return `<div style="display: flex; align-items: center; gap: 5px; margin-right: 15px;">
              <div style="width: 20px; text-align: center; font-weight: bold; font-size: 12px;">${index + 1}</div>
              <div class="blue-ball">${item.number}</div>
              <div style="font-size: 12px; color: #666;">${accuracyText}%</div>
            </div>`;
          }).join(' ');
          ...
          // 生成图表
          const chartCanvas = document.getElementById('buyNumberTotalChartFifthLastBack');
          if (chartCanvas) {
            // 清空之前的图表
            ...
            new Chart(ctx, {
              ...
              data: {
                ...
                datasets: [{
                  label: '平均正确率累加',
                  data: data,
                  backgroundColor: 'rgba(33, 84, 240, 0.6)',
                  borderColor: 'rgba(33, 84, 240, 1)',
                  borderWidth: 1
                }]
              },
              options: {
                  ...
                  tooltip: {
                    callbacks: {
                      label: function(context) {
                        return `正确率: ${context.parsed.y.toFixed(3)}%`;
                      }
                    }
                  }
                }
          ...
          totalRankingContainer.innerHTML = '<div style="color: #666;">暂无推荐买号数据</div>';
      ...
      console.error('生成后区推荐买号总集失败:', error);
    }
  }
    ...
    try {
      ...
      for (const backtestMethod of backtestMethods) {
        ...
        for (let stageIndex = 0; stageIndex < searchStages.length; stageIndex++) {
          ...
          // 并行处理，每次处理5个统计期
          const batchSize = 5;
          for (let batchStart = 0; batchStart < testPeriods.length; batchStart += batchSize) {
            // 检查是否需要终止回测
            if (isBacktestStopped) {
              break;
            }
            
            const batchEnd = Math.min(batchStart + batchSize, testPeriods.length);
            const currentBatch = testPeriods.slice(batchStart, batchEnd);
            
            // 实时更新进度（每批次更新一次）
            stageCount = batchEnd;
            const loopProgress = 15 + ((currentStep - 1 + (stageCount / stageTotal)) / totalSteps) * 60;
            await updateProgress(
              `正在测试统计期${currentBatch[0]}-${currentBatch[currentBatch.length - 1]}期..`, 
              loopProgress, 
              `测试期数: ${stageCount}/${stageTotal}`
            );
            
            // 并行执行当前批次的测试
            const batchResults = await Promise.all(
              currentBatch.map(i => testStatsPeriod(backtestPeriod, i, backtestMethod, true))
            );
      ...
      if (isBacktestStopped) {
        ...
        for (let i = 0; i < allResults.length; i++) {
          ...
          result.backBuyNumbers = await huo_qu_hou_qu_tui_jian_mai_hao(result.statsPeriod, result.backtestMethod, true);
        ...
        if (isBacktestStopped) {
          ...
          // 过滤回测结果：如果回测方法相同且下下期后区推荐买号相同，只保留一条
          const uniqueResultsMap = new Map();
          
          validResults.forEach(result => {
            // 基于回测方法和下下期后区推荐买号（排序后）创建唯一键
            const sortedBuyNumbers = [...(result.backBuyNumbers || [])].sort((a, b) => a - b);
          ...
          await renderDetailDataSecondLastBack(filteredResults, nextPeriod, backtestPeriod);
  ...
  async function performSearchThirdLastBack() {
    ...
    try {
      ...
      // 获取下下下期期号
      const nextPeriod = await getLatestPeriodThirdLast(); // 使用sql_zui_xin_yi_qi接口获取下下下期期号
      ...
      for (const backtestMethod of backtestMethods) {
        ...
        for (let stageIndex = 0; stageIndex < searchStages.length; stageIndex++) {
          ...
          for (let batchStart = 0; batchStart < testPeriods.length; batchStart += batchSize) {
            ...
            // 并行执行当前批次的测试
            const batchResults = await Promise.all(
              currentBatch.map(i => testStatsPeriod(backtestPeriod, i, backtestMethod, false, true))
            );
      ...
      if (isBacktestStopped) {
        ...
        for (let i = 0; i < allResults.length; i++) {
          ...
          result.backBuyNumbers = await huo_qu_hou_qu_tui_jian_mai_hao(result.statsPeriod, result.backtestMethod, false, true);
        ...
        if (isBacktestStopped) {
          ...
          await renderDetailDataThirdLastBack(filteredResults, nextPeriod, backtestPeriod);
  ...
  async function performSearchFourthLastBack() {
    ...
    try {
      ...
      // 获取下下下下期期号
      const nextPeriod = await getLatestPeriodFourthLast(); // 使用sql_zui_xin_yi_qi接口获取下下下下期期号
      ...
      for (const backtestMethod of backtestMethods) {
        ...
        for (let stageIndex = 0; stageIndex < searchStages.length; stageIndex++) {
          ...
          for (let i = currentStart; i <= currentEnd; i += step) {
            ...
            const result = await testStatsPeriod(backtestPeriod, i, backtestMethod, false, false, true);
            stageResults.push(result);
            allTestResults.push(result);
          }
      ...
      if (isBacktestStopped) {
        ...
        for (let i = 0; i < allResults.length; i++) {
          ...
          const result = allResults[i];
          let methodName;
          if (result.backtestMethod.startsWith('rank')) {
            const rankMatch = result.backtestMethod.match(/^rank(\d+)$/);
          ...
          result.backBuyNumbers = await huo_qu_hou_qu_tui_jian_mai_hao(result.statsPeriod, result.backtestMethod, false, false, true);
        ...
        if (isBacktestStopped) {
          ...
          // 渲染详情数据，传递过滤和排序后的结果和下下下下期期号
          await renderDetailDataFourthLastBack(filteredResults, nextPeriod, backtestPeriod);
  ...
  async function performSearchFifthLastBack() {
    ...
    try {
      ...
      // 获取下下下下下期期号
      const nextPeriod = await getLatestPeriodFifthLast(); // 使用sql_zui_xin_yi_qi接口获取下下下下下期期号
      ...
      for (const backtestMethod of backtestMethods) {
        ...
        for (let stageIndex = 0; stageIndex < searchStages.length; stageIndex++) {
          ...
          for (let i = currentStart; i <= currentEnd; i += step) {
            ...
            const result = await testStatsPeriod(backtestPeriod, i, backtestMethod, false, false, false, true);
            stageResults.push(result);
            allTestResults.push(result);
          }
      ...
      if (isBacktestStopped) {
        ...
        for (let i = 0; i < allResults.length; i++) {
          ...
          result.backBuyNumbers = await huo_qu_hou_qu_tui_jian_mai_hao(result.statsPeriod, result.backtestMethod, false, false, false, true);
        ...
        if (isBacktestStopped) {
          ...
          // 渲染详情数据，传递过滤和排序后的结果和下下下下下期期号
          await renderDetailDataFifthLastBack(filteredResults, nextPeriod, backtestPeriod);
  ...
  // 渲染详情数据 - 近两期两球组合后区
  async function renderDetailDataNearTwoPeriodsBack(allResults, nextPeriod) {
    ...
    try {
      ...
      if (isBacktestStopped) {
        ...
        if (isBacktestStopped) {
          ...
          // 渲染详情数据，传递所有结果和下一期期号
          await renderDetailDataNearTwoPeriodsBack(allResults, nextPeriod);
